                ...
                template.hasResourceProperties("AWS::IAM::Policy", {
                    "PolicyDocument": {
                        "Statement": Match.arrayWith([{
                            ...
                            Effect: "Allow",
                            Resource: Match.stringLikeRegexp(".*:parameter/greenmo/.*")
                        }]),
                    },
                    "Roles": Match.arrayWith([{
                        Ref: Match.stringLikeRegexp("^chargableCarsLambda.*"),
                    }]),
                });
            });
        ...
        describe('Then a API Gateway', () => {
            ...
            test('Should be integrated with the Lambda Function', () => {
                template.hasResourceProperties("AWS::ApiGateway::Method", {
                    Integration: {
                        Type: "AWS_PROXY",
                        Uri: {
                            "Fn::Join": Match.arrayWith([Match.arrayWith([Match.objectLike({
                                "Fn::GetAtt": Match.arrayWith([Match.stringLikeRegexp("^chargableCarsLambda.*")]),
                            })])])
                        }
                    }                       
                });
            ...
            test('Should have a Usage Plan with API Key', () => {
                template.hasResourceProperties("AWS::ApiGateway::UsagePlan", {
                    ApiStages: Match.arrayWith([Match.objectLike({
                        ApiId: {
